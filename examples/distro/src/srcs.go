...
func SrcLayer(name string, extras ...LayerSpecOpt) Spec {
	mountDir := filepath.Join("/src", name)
	return Wrap(LayerSpec(
		Name(name),
...
func (s ViaCurl) Spec() Spec {
	opts := []LayerSpecOpt{s.AlwaysRun, BuildScript(
		`mkdir -p /src`,
		`cd /src`,
	...
	if !s.NoOverride {
		opts = append(opts, LocalOverride(false))
	}
	return SrcLayer(s.Name, opts...)
}
...
func (s ViaGit) Spec() Spec {
	opts := []LayerSpecOpt{s.AlwaysRun, BuildScript(
		...
		`cd /src`,
		fmt.Sprintf(`git checkout -f %s`, s.Ref),
	)}
	if !s.NoOverride {
		opts = append(opts, LocalOverride(false))
	}
...
func (CACerts) Spec() Spec {
	return ViaGit{
		URL:  "https://github.com/djlucas/make-ca.git",
		Ref:  "v1.7",
		Name: "cacerts-src",
	}.Spec()
}
